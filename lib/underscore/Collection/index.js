...
;(function _map() {
  ...
  assert(returnValue.every(function every(element, index, array) {
    return element === index + collection[index]
  }))
  ...
  assert(_.isArray(returnValue))
  keys = _.keys(collection)
  assert(returnValue.length === keys.length)
  assert(returnValue.every(function (key) {
