...
;(function _keysAndAllKeys() {
  ...
  assert(_.indexOf(ownKeys, 'obj-enum') !== -1)

  // _.allKeys(object)
  allKeys = _.allKeys(obj)
  assert(allKeys.length === 2)
  assert(_.indexOf(allKeys, 'obj-enum') !== -1)
  assert(_.indexOf(allKeys, 'proto-enum') !== -1)
...
;(function _values() {
  ...
  assert(_.indexOf(values, 'obj-enum') !== -1)
...
;(function _mapObject() {
  ...
  keys = _.keys(transformedObj)
  assert(keys.length === 2)
  assert(keys.every(function every(key) {
    return transformedObj[key] === obj[key] + key.length
  }))
...
;(function _pairs() {
  ...
  assert(pairs.length === keys.length)
  assert(pairs.every(function every(pair) {
    return Array.isArray(pair)
      && pair.length === 2
      && _.indexOf(keys, pair[0]) !== -1
      && obj[pair[0]] === pair[1]
  }))
...
;(function _invert() {
  ...
  assert(objValues.length === invertedObjKeys.length)
  assert(invertedObjKeys.every(function every(key) {
    return obj[invertedObj[key]] === key
  }))
...
;(function _create() {
  ...
  assert(objKeys.length === propsKeys.length)
  assert(objKeys.every(function every(key) {
    return _.indexOf(propsKeys, key) !== -1 && obj[key] === props[key]
  }))
...
;(function _methods() {
  ...
  assert(methods.every(function every(method) {
    return obj.hasOwnProperty(method) && typeof method === 'function'
  }))
...
;(function _extendAndExtendOwn() {
  ...
  var s1Keys = _.keys(source1)
  var s2Keys = _.keys(source2)
  var protoKeys = _.keys(proto)

  var extendKeys = _.keys(obj1)
  var extendOwnKeys = _.keys(obj2)

  // _.extend(destination, *sources)
  assert(extendKeys.length === _.uniq(_.union(s1Keys, s2Keys, protoKeys)).length)
  assert(extendKeys.every(function every(key) {
    if (key in source2) {
      return obj1[key] === source2[key]
    } else {
      return obj1[key] === source1[key]
    }
  }))

  // _.extendOwn(destination, *sources)
  assert(extendOwnKeys.length === _.uniq(_.union(s1Keys, s2Keys)).length)
  assert(extendOwnKeys.every(function every(key) {
    if (key in source2) {
      return obj1[key] === source2[key]
    } else {
      return obj1[key] === source1[key]
...
;(function _pick() {
  ...
  keys = _.keys(filteredObj)
  assert(keys.length === 1)
  assert(keys[0] === filterKey)
...
;(function _omit() {
  ...
  keys = _.keys(filteredObj)
  assert(keys.length === _.keys(obj).length - 1)
  assert(!(filterKey in filteredObj))
  assert(keys.every(function every(key) {
    return key in obj
  }))
...
;(function _defaults() {
  ...
  var filledObj = _.defaults(obj, defaults)

  var objKeys = _.keys(obj)
  var defaultsKeys = _.keys(defaults)
  var filledObjKeys = _.keys(filledObj)

  assert(filledObjKeys.length === _.uniq(_.union(objKeys, defaultsKeys)).length)
  assert(filledObjKeys.every(function every(key) {
    if (obj.hasOwnProperty(key)) {
      return filledObj[key] === obj[key]
    } else {
      return filledObj[key] === defaults[key]
    }
  }))
...
;(function _clone() {
  ...
  var nKeys = _.keys(n)
  var cloneKeys = _.keys(clone)

  assert(cloneKeys.length === nKeys.length)
  assert(cloneKeys.every(function every(key) {
    return _.indexOf(nKeys, key) !== -1 && clone[key] === n[key]
  }))
