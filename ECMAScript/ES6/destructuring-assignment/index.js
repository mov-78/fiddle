...
//
{
  ...
  console.assert(null === d)      // 匹配成功（null !== undefined）
  console.assert('e' === e)       // 匹配成功（递归匹配）
  console.assert(undefined === f) // 匹配失败，默认值为 undefined
...
//
{
  ...
  console.assert('b' === c) // 匹配成功；其中 b 键仅用于匹配
  console.assert('d' === d) // 匹配失败，取默认值
  console.assert('e' === f) // 匹配成功，忽略默认值；默认值表达式惰性求值，仅在不匹配时才会对其求值
